...
void ToolMain::UpdateInput(MSG* msg)
{
	...
	if (m_toolInputCommands.mouse_LB_Down && !m_toolInputCommands.shiftDown)
	{
		...
		// Apply texture to the selected object
		m_textureManager.get()->ApplyTextureToObject();

	
	}
	else if (m_toolInputCommands.mouse_LB_Down && m_toolInputCommands.shiftDown)
	{ 
            // Clear selected objects and remove texture
			m_textureManager.get()->selectedObjects.clear(); m_textureManager.get()->RemoveTexture();
